...
**Статус**: later
**Цель**: не держать 2B и старшую модель одновременно; прогонять документы «волнами» (сначала все router, затем все extract) для слабого железа.
**Идея**:

- Фаза A: загрузить 2B → прогнать классификацию/детектор → выгрузить.
- Фаза B: загрузить SAGE/8B → прогнать split/extract → выгрузить.

### 1.2 Эвристика small-batch: пропуск 2B

**Статус**: later
**Цель**: если документов мало (2–3), то выгоднее сразу грузить «старшую» модель, чем тратить время на загрузку/перегрузку 2B.
...
**Идея**: VLM возвращает bbox; Sharp режет; оркестратор кладет sub-docs в staging и прогоняет их как обычные документы.
